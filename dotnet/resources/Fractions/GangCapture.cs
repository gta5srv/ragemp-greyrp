        ...
        private static void endCapture(GangPoint region, int defenders, int attackers)
        {
            ...
            if (attackers <= defenders)
            {
                ...
                foreach (var m in Manager.Members.Keys)
                {
                    ...
                    {
                        MoneySystem.Wallet.Change(m, 50);
                        GameLog.Money($"server", $"player({Main.Players[m].UUID})", 50, $"winCapture");
                    }
                }
            }
            else if (attackers > defenders)
            {
                ...
                foreach (var m in Manager.Members.Keys)
                {
                    ...
                    {
                        MoneySystem.Wallet.Change(m, 25);
                        GameLog.Money($"server", $"player({Main.Players[m].UUID})", 25, $"winCapture");
                    }
                }
            }
            DateTime nextcapt = DateTime.Now.AddMinutes(config.TryGet<double>("nextcapt", 30));
