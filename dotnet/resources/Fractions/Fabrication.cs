...
namespace UnitedRP.Fractions
{
    class AlcoFabrication : Script
    {
        ...
        */
        ...
        {
            //{ 10, new Vector3(-291.25262, -430.2757, 29.11753) },
            //{ 12, new Vector3(-21.22596, 240.8915, -108.4333) },
            //{ 13, new Vector3(-430.1028, 261.2774, 81.88689) },
        };
        private static Dictionary<int, Vector3> ExitAlcoShop = new Dictionary<int, Vector3>()
        {
            //{ 10, new Vector3(1578.2656, 252.9592, -47.12514) },
            //{ 12, new Vector3(-21.22596, 240.8915, -108.4333) },
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                //NAPI.Blip.CreateBlip(674, new Vector3(-291.25262, -430.2757, 29.11753), 0.7f, 0, "Augury Club", 255, 0, true);
                //NAPI.Blip.CreateBlip(121, new Vector3(141.3792, -1281.576, 28.2172), 0.7f, 0, "Ванилла", 255, 0, true);
                //NAPI.Blip.CreateBlip(136, new Vector3(-21.22596, 240.8915, -108.4333), 0.7f, 0, "Galaxy Club", 255, 0, true);
                //NAPI.Blip.CreateBlip(205, new Vector3(-430.1028, 261.2774, 81.88689), 0.7f, 0, "Comedy Club", 255, 0, true);
                ...
                foreach (var pair in ExitAlcoShop)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~b~Выход", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region Unloadpoints
                foreach (var pair in UnloadPoints)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel($"~b~Выгрузить материалы", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region BuyPoints
                foreach (var pair in BuyPoints)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~b~Барная карта", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
            }
        ...
        public static void ResistTimer(Player player)
        {
            ...
            if (player.GetData<int>("RESIST_TIME") == 0)
            {
                ...
                Trigger.PlayerEvent(player, "setResistStage", 0);
        ...
        public static void OpenBuyAlcoholMenu(Player player)
        {
            ...
            Trigger.PlayerEvent(player, "openAlco", club, ClubsStocks[club].PriceModifier, isOwner, stock);
        }
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case 2: // craft
                        ...
                        return;
                    case 3: // setprice
                        Trigger.PlayerEvent(player, "openInput", "Установить цену", "Введите цену для алкоголя в процентах", 3, "club_setprice");
                        return;
                }
            }
        ...
        internal class Stock
        {
            ...
            {
                Label = NAPI.TextLabel.CreateTextLabel($"~b~Материалы: {mats}\nАлкоголь: {a1 + a2 + a3}", pos, 30f, 0.3f, 0, new Color());
            ...
            {
                Label.Text = $"~b~Материалы: {Materials}\nАлкоголь: {Alco1 + Alco2 + Alco3}";
            }
        }
    }
}
