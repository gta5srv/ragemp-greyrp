...
namespace UnitedRP.Fractions
{
    class MafiaWars : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                foreach (var vec in warPoints)
                {
                    ...
                    warBlips.Add(i, NAPI.Blip.CreateBlip(543, vec, 0.7f, 40, Main.StringToU16("Война за крышевание"), 255, 0, true, 0, 0));
                    i++;
                }
            }
        ...
        public static void CMD_takeBiz(Player player)
        {
            ...
            if (Main.Players[player].AdminLVL < 5)
                if (Main.Players[player].FractionID == 0 || !Fractions.Manager.isLeader(player, Main.Players[player].FractionID))
                 return;
        ...
        public static void CMD_startBizwar(Player player)
        {
            ...
            if (Manager.countOfFractionMembers(biz.Mafia) < 3)
            {
            ...
            //toStartWarTimer = Timers.StartOnce(5000, () => timerStart());
            warStarting = true;
            smbTryCapture = false;
        }
        ...
        private static void timerStart()
        {
            ...
            foreach (var p in NAPI.Pools.GetAllPlayers())
            {
                ...
                {
                    Trigger.PlayerEvent(p, "sendCaptureInformation", attackers, defenders, 0, 0);
        ...
        private static void timerUpdate()
        {
            try
            {
                var attackers = 0;
                var defenders = 0;
                BCore.Bizness biz = BCore.BizList[bizID];
                foreach (var p in NAPI.Pools.GetAllPlayers())
                {
                    if (!Main.Players.ContainsKey(p) || !p.HasData("WARZONE") || p.GetData<int>("WARZONE") != whereWarIsGoing) continue;
                    if (Main.Players[p].FractionID == biz.Mafia) defenders++;
                ...
                foreach (var p in NAPI.Pools.GetAllPlayers())
                {
                    ...
                    if (Main.Players[p].FractionID == biz.Mafia || Main.Players[p].FractionID == attackersFracID)
                    {
                        Trigger.PlayerEvent(p, "sendCaptureInformation", attackers, defenders, minutes, seconds);
                    }
                }
            }
        ...
        private static void endCapture()
        {
            ...
            try
            {
                ...
                Main.PlayerEventToAll("captureHud", false);
                var biz = BCore.BizList[bizID];
                ...
                if (attackersSt <= defendersSt)
                {
                    ...
                    foreach (var m in Manager.Members.Keys)
                    {
                        ...
                        {
                            MoneySystem.Wallet.Change(m, 150);
                            GameLog.Money($"server", $"player({Main.Players[m].UUID})", 150, $"winBiz");
                        }
                    }
                }
                else if (attackersSt > defendersSt)
                {
                    ...
                    foreach (var m in Manager.Members.Keys)
                    {
                        ...
                        {
                            MoneySystem.Wallet.Change(m, 75);
                            GameLog.Money($"server", $"player({Main.Players[m].UUID})", 75, $"winBiz");
                        }
                    }
                    biz.UpdateLabel();
                }
        ...
        private static void onPlayerEnterBizWar(ColShape shape, Player player)
        {
            try
            {
                ...
                if (Main.Players[player].FractionID >= 10 && Main.Players[player].FractionID <= 13)
                {
                    ...
                    if (warIsGoing && (Main.Players[player].FractionID == attackersFracID || Main.Players[player].FractionID == BCore.BizList[bizID].Mafia) && whereWarIsGoing == shape.GetData<int>("ID"))
                    {
                        int minutes = timerCount / 60;
                        int seconds = timerCount % 60;
