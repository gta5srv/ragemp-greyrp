...
namespace NeptuneEVO.Jobs
{
    ...
    {
        private static int checkpointPayment = 5;
        ...
        public void Event_ResourceStart()
        {
            try
            {
                //NAPI.TextLabel.CreateTextLabel("~b~", new Vector3(724.8585, 134.1029, 81.95643), 30f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Начать/Закончить работы"), new Vector3(724.9625, 133.9959, 80.95643), 30f, 0.4f, 0, new Color(255, 255, 255), true, 0);
                ...
                {
                    col = NAPI.ColShape.CreateCylinderColShape(Check.Position, 1.2f, 4, 0);
        ...
        public static void StartWorkDay(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не работаете электриком. Устроиться можно у начальника", 3000);
                return;
            }
            ...
            else
            {
                ...
                Trigger.PlayerEvent(player, "createCheckpoint", 15, 1, Checkpoints[check].Position, 1, 0, 255, 0, 0);
        ...
        private static void PlayerEnterCheckpoint(ColShape shape, Player player)
        {
            try
            {
                ...
                var payment = Convert.ToInt32((checkpointPayment + Main.AddJobPoint(player) * 3) * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);
                ...
                player.SendNotification($"Электрик: ~h~~g~+{payment + level}$", true);
                ...
                NAPI.Task.Run(() => {
                    try
                    {
                        if (player != null && Main.Players.ContainsKey(player))
                        {
                            ...
                            Trigger.PlayerEvent(player, "createCheckpoint", 15, 1, Checkpoints[nextCheck].Position, 1, 0, 255, 0, 0);
                            Trigger.PlayerEvent(player, "createWorkBlip", Checkpoints[nextCheck].Position);
                        }
                    }
                    catch { }
                }, 4000);
