...
{ 
    class AutoMechanic : Script
    {
        ...
        public static void mechanicRepair(Player player, Player target, int price)
        {
            ...
            Trigger.PlayerEvent(target, "openDialog", "REPAIR_CAR", $"Игрок ({player.Value}) предложил отремонтировать Ваш транспорт за ${price}");
                ...
                if (seatid == -1)
                {
                    ...
                    if (Main.Players[player].WorkID == 8)
                    {
                        if (NAPI.Data.GetEntityData(player, "WORK") == null)
                        {
                            ...
                            {
                                Trigger.PlayerEvent(player, "openDialog", "MECHANIC_RENT", $"Арендовать рабочий транспорт за ${mechanicRentCost}?");
                            }
                            else
                            {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Это выставочный транспорт", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
            }
        ...
        public static void callMechanic(Player player)
        {
            if (!NAPI.Data.GetEntityData(player, "IS_CALL_MECHANIC"))
            {
                ...
                foreach (var p in players)
                {
                    ...
                    {
                        i++;
                        NAPI.Chat.SendChatMessageToPlayer(p, $"~b~[ДИСПЕТЧЕР]: ~w~Игрок ({player.Value}) вызвал автомеханика ~y~({player.Position.DistanceTo(p.Position)}м)~w~. Напишите ~y~/ma ~b~[ID]~w~, чтобы принять вызов");
                    }
                }
                if (i > 0)
                {
        ...
        public static void buyFuel(Player player, int fuel)
        {
            ...
            if (player.GetData("BIZ_ID") == -1 || BCore.BizList[player.GetData("BIZ_ID")].Type != 1)
            {
        ...
        public static void mechanicFuel(Player player, Player target, int fuel, int pricePerLitr)
        {
            ...
            Trigger.PlayerEvent(target, "openDialog", "FUEL_CAR", $"Игрок ({player.Value}) предложил заправить Ваш транспорт на {fuel}л за ${fuel * pricePerLitr}");
