...
namespace NeptuneEVO.Jobs
{
    ...
    {
        static int checkpointPayment = 4;
        ...
        public void onResourceStartHandler()
        {
            try
            {
                ...
                ColShape Shape = NAPI.ColShape.CreateCylinderColShape(new Vector3(-1332.242, 59.39482, 52.57008), 1F, 2F);
                NAPI.Marker.CreateMarker(27, new Vector3(-1332.242, 59.39482, 52.57008) + new Vector3(0, 0, 0.2f), new Vector3(), new Vector3(), 1f, new Color(0, 86, 214, 220), false, 0);
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Взять транспорт"), new Vector3(-1332.242, 59.39482, 52.57008 + 1.2), 30f, 0.5f, 0, new Color(255, 255, 255));
                Shape.OnEntityEnterColShape += (s, entity) =>
                {
                    try
                    {
                        NAPI.Data.SetEntityData(entity, "INTERACTIONCHECK", 228);
                    }
        ...
        public void onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не работаете газонокосильщиком. Устроиться можно у начальника", 3000);
                    VehicleManager.WarpPlayerOutOfVehicle(player);
                }
            }
        ...
        {
            new Vector3(-1330.686, 42.13113, 52.44025),
            new Vector3(-1331.43, 46.86374, 52.44976),
            new Vector3(-1332.323, 53.43707, 52.4815),
        };
        ...
        {
            var veh = NAPI.Vehicle.CreateVehicle(VehicleHash.Mower, getMowerPos(), 235, 4, 4, "MOWER");
        ...
        private static void mowerCheckpointEnterWay(ColShape shape, Player player)
        {
            try
            {
                ...
                var payment = Convert.ToInt32((checkpointPayment + Main.AddJobPoint(player) * 2) * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);

                player.SetData("PAYMENT", player.GetData<int>("PAYMENT") + payment);

                player.SendNotification($"Косилка: ~h~~g~+{payment}$", true);
                ...
                {
                    var next_way = 0;
                    Trigger.PlayerEvent(player, "createCheckpoint", 4, 1, MowerWays[next_way][0] - new Vector3(0, 0, 1.12), 2, 0, 0, 0, 255, MowerWays[next_way][1] - new Vector3(0, 0, 1.12));
