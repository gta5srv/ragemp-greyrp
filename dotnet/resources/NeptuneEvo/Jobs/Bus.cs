...
namespace NeptuneEVO.Jobs
{
    class Bus : Script
    {
        ...
        public static int BusRentCost = 35;
        ...
        public void onResourceStartHandler()
        {
            try
            {
                ...
                foreach (var station in BusStations)
                    NAPI.TextLabel.CreateTextLabel($"~y~Остановка\n~b~{station.Key}", station.Value, 30f, 0.4f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);

                ColShape Shape = NAPI.ColShape.CreateCylinderColShape(new Vector3(450.5199, -654.3629, 27.26488), 1F, 2F);
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Взять автобус"), new Vector3(450.5199, -654.3629, 27.26488 + 1.2), 30f, 0.5f, 0, new Color(255, 255, 255));
        ...
        private static void busCheckpointEnterWay(ColShape shape, Player player)
        {
            try
            {
                ...
                if (!BusWays[way][check].IsStop)
                {
                    ...
                    {
                        int costformash = (int)Math.Floor( (2000 + 100 * Main.GetJobPoints(player) ) * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);
            ...
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    if (NAPI.Data.GetEntityData(player, "CAR_EXIT_TIMER_COUNT") > 60)
                    {
                        ...
                        Utils.QuestsManager.AddQuestProcess(player, 5, player.GetData<int>("PAYMENT"));
