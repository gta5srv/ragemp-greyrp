...
using UnitedRP.Houses;
using UnitedRP.Jobs;
using System.Data.SQLite;
using UnitedRP.Businesses;
using UnitedRP.Organization;
using static UnitedRP.Core.VehicleStreaming;
                ...
                else
                {
                    switch (id)
                    {
                        case 0:
							...
							if (Players[player].FamilyRank > 0) UnitedRP.Families.Member.LoadMembers(player, Players[player].FamilyCID, Players[player].FamilyRank);
                        ...
                        case 1:
                            ...
                            break;
                        case 3:
                            UnitedRP.Families.Family family = UnitedRP.Families.Family.GetFamilyToCid(player);
...
[ServerEvent(Event.PlayerEnterVehicle)]
                    ...
                    case "setMfamily":
                        {
                            ...
                            UnitedRP.Families.Family fam = UnitedRP.Families.Family.GetFamilyToCid(player);
        ...
        public async Task ClientEvent_signup(Player player, params object[] arguments)
        {
            try
            {
                ...
                if (ban != null)
                {
                    if (ban.isHard && ban.CheckDate())
                    {
                        NAPI.Task.Run(() => Trigger.PlayerEvent(player, "kick", $"Вы заблокированы до {ban.Until}. Причина: {ban.Reason} ({ban.ByAdmin})"));
                        return;
                    }
                }
                ...
                Log.Write("signup: " + e.Message, nLog.Type.Error); 
            }
        }
        #endregion Account
        ...
        public void PlayerEvent_engineCarPressed(Player player, params object[] arguments)
        {
            ...
            { 
                Log.Write("engineCarPressed: " + e.Message, nLog.Type.Error); 
            }
        }
        ...
        public void PlayerEvent_lockCarPressed(Player player, params object[] arguments)
        {
            ...
            { 
                Log.Write("lockCarPressed: " + e.Message, nLog.Type.Error); 
            }
        }
            ...
            try
            {
                ...
                switch (id)
                {
					...
					case 803: //todo FamilyCreatorMenu
                        UnitedRP.Families.Manager.OpenCreatorFamilyMenu(player);
                        return;	
                    case 500:
                        ...
                        case "INVITED":
							...
							case "INVITEDTOFAMILY":
                            {
                                try
                                {
                                    ...
                                    UnitedRP.Families.Member.LoadMembers(player, familycid, 1);
                                    UnitedRP.Families.Family family = UnitedRP.Families.Family.GetFamilyToCid(familycid);
                                    family.Players.Add(new UnitedRP.Families.Member(player.Name.ToString(), family.Name, family.FamilyCID, 1, UnitedRP.Families.Ranks.GetFamilyRankName(family.FamilyCID, 1)));

                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы вступили в {UnitedRP.Families.Family.GetFamilyName(player.GetData<Player>("SENDERFAMILY"))}", 3000);
                                }
                                catch (Exception e)
                                {
        ...
        public static void payDayTrigger()
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        UnitedRP.Families.Components.StartToWar();
                    }
                    ...
                    foreach (Houses.House h in Houses.HouseManager.Houses)
                    {
                        try
                        {
                            ...
                            if (UnitedRP.Families.Family.FalimyHouses.ContainsKey(h.ID)) continue;
