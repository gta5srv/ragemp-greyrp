...
"SELECT * FROM `ColShapes`";
        ...
        private void LoadFromDB(DataRow row)
        {
            ...
            if (tp.Interact)
            {
                tp.Tp[0].OnEntityEnterColShape += (ColShape c, Player p) =>
                {
                    p.SetData("ColShapeInteract", tp.SID);
                };
                tp.Tp[0].OnEntityExitColShape += (ColShape c, Player p) =>
                {
        ...
        public bool CheckTeleportAccess(Player player, string sid)
        {
