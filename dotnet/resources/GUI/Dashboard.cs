...
namespace UnitedRP.GUI
{
    class Dashboard : Script
    {
        ...
        public void PlayerEvent_Inventory(Player player, params object[] arguments)
        {
            try
            {
                        ...
                        {// self inventory
                            ...
                            {//remove one item from player inventory
                                ...
                                if (item.Count > 1)
                                {
                                    ...
                                    Trigger.PlayerEvent(player, "openInput", "Выбросить предмет", "Введите количество", 3, "item_drop");
                                    return;
                                }
                                    ...
                                    case 2:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.PlayerEvent(player, "openInput", "Переложить предмет", "Введите количество", 3, "item_transfer_toveh");
                                                return;
                                            }
                                    ...
                                    case 3:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.PlayerEvent(player, "openInput", "Переложить предмет", "Введите количество", 3, "item_transfer_tosafe");
                                                return;
                                            }
                                    ...
                                    case 5:
                                        {
                                            ...
                                            if (item.Type == ItemType.BagWithDrill)
                                            {
                                                ...
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У Гражданина уже есть дрель или деньги в руках", 3000);
                                                    return;
                                                }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.PlayerEvent(player, "openInput", "Передать предмет", "Введите количество", 3, "item_transfer_toplayer");
                                                return;
                                            }
                                    ...
                                    case 6:
                                        {
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.PlayerEvent(player, "openInput", "Передать предмет", "Введите количество", 3, "item_transfer_tofracstock");
                                                return;
                                            }
                                    ...
                                    case 7:
                                        {
                                            ...
                                            if (keys.Count >= 40)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Максимум 40 ключей", 3000);
                                                return;
                                            }
                    ...
                    case 3: // Взять
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.PlayerEvent(player, "openInput", "Взять предмет", "Введите количество", 3, "item_transfer_fromsafe");
                                return;
                            }
                    ...
                    case 6:
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.PlayerEvent(player, "openInput", "Взять предмет", "Введите количество", 3, "item_transfer_fromfracstock");
                                return;
                            }
                            ...
                            Fractions.Manager.FracLogs[Main.Players[player].FractionID].Add(new List<object> { DateTime.Now.ToString("dd.MM.yyyy"), $"{DateTime.Now.Hour}:{(DateTime.Now.Minute < 10 ? "0" : "" )}{DateTime.Now.Minute}", player.Name, $"{nInventory.ItemsNames[(int)item.Type]}{serial}", 1 });
                    ...
                    case 20: 
                        {
                            if(Main.Players[player].AdminLVL >= 6 && Main.Players[player].InsideHouseID == -1) {
                                ...
                                if (nInventory.ClothesItems.Contains(item.Type) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Гражданин должен снять эту одежду", 3000);
                                    return;
                                }
                                else if ((nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type)) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Гражданин должен убрать это оружие из рук", 3000);
                                    return;
                                }
        ...
        public void PlayerEvent_openInventory(Player player, params object[] arguments)
        {
            try
            {
        ...
        public static void Close(Player player, bool resetOpenOut = false)
        {
            ...
            Trigger.PlayerEvent(player, "board", data);
            player.ResetData("OPENOUT_TYPE");
        }
        ...
        public static void sendStats(Player player)
        {
            try
            {
                ...
                int lvljob = Main.GetJobPoints(player);
                string tims = GetPlayerTimeWheell(player);
                string time = GetPlayerTime(player);
                string lvl = Convert.ToString(lvljob);
                if (lvljob == -1)
                    lvl = "0";
                List<object> data = new List<object>
                {
                    ...
                    lvl,
                    time,
                    Main.Players[player].LuckyWheell,
                    tims
                ...
                Trigger.PlayerEvent(player, "board", 2, json);
            ...
            try
            {
                ...
                foreach (nItem item in items)
                {
                    List<object> idata = new List<object>
                    {
                        ...
                        item.Count,
                        (item.IsActive) ? 1 : 0,
                        (nInventory.WeaponsItems.Contains(item.Type) || item.Type == ItemType.StunGun) ? "" + item.Data : (item.Type == ItemType.CarKey) ? $"{(string)item.Data.Split('_')[0]}" : ""
                    };
                    data.Add(idata);
                }
                ...
                string json = JsonConvert.SerializeObject(data);
                Log.Debug(json);
                Trigger.PlayerEvent(player, "board", 3, json);
        ...
        public static async Task SendItemsAsync(Player player)
        {
            try
            {
                ...
                NAPI.Task.Run(() => Trigger.PlayerEvent(player, "board", 3, json));
        ...
        {
            Trigger.PlayerEvent(Player, "board", 0);
        }
            ...
            try
            {
                ...
                foreach (nItem item in items)
                {
                    List<object> idata = new List<object>
                    {
                        ...
                        item.Count,
                        (item.IsActive) ? 1 : 0,
                        (nInventory.WeaponsItems.Contains(item.Type) || item.Type == ItemType.StunGun) ? "" + item.Data : (item.Type == ItemType.CarKey) ? $"{item.Data}" : ""
                    };
                    Items.Add(idata);
                }
                data.Add(Items);
                ...
                Trigger.PlayerEvent(Player, "board", 4, json);
                Trigger.PlayerEvent(Player, "board", 5, true);
                Trigger.PlayerEvent(Player, "board", 0);
            }
            catch (Exception e)
            {
        ...
        public static void Update(Player Player, nItem item, int index)
        {
            ...
            Trigger.PlayerEvent(Player, "board", 6, json, index);
        }
        public static Task UpdateAsync(Player Player, nItem item, int index)
        {
            try
            {
                ...
                string json = JsonConvert.SerializeObject(idata);
                NAPI.Task.Run(() => Trigger.PlayerEvent(Player, "board", 6, json, index));
            }
