...
using UnitedRP.Businesses;

namespace UnitedRP.MoneySystem
{
    class Bank : Script
    {
    ...
    class ATM : Script
    {
        ...
        public static void AtmCompanyGen(Player player)
        {
            var acc = Main.Players[player];
            if (!string.IsNullOrEmpty(acc.Org) && Organization.OCore.OrgListNAME[acc.Org].Owner == player.Name)
            {
                List<string> data = new List<string> { "Перевести на счёт", "Перевести со счёта" };
                Trigger.PlayerEvent(player, "atmOpenComp", JsonConvert.SerializeObject(data), "");
            }
            else
            {
                Trigger.PlayerEvent(player, "atmOpen", "[1,0,0]");
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У вас нет компании!", 3000);
            }
        }
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case 4:
                        ...
                        break;
                    case 42: // вывод
                        if (string.IsNullOrEmpty(acc.Org)) return;
                        Bank.Data banks = Bank.Accounts[Organization.OCore.OrgListNAME[acc.Org].BankID];
                        if (banks.Balance < +Math.Abs(amount))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Не достаточно средств!", 3000);
                            return;
                        }
                        MoneySystem.Wallet.Change(player, +Math.Abs(amount));
                        MoneySystem.Bank.Change(Organization.OCore.OrgListNAME[acc.Org].BankID, -Math.Abs(amount));
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы вывели из счёта компании: {amount}$!", 3000);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"company({acc.Org})", Math.Abs(amount), $"atmCompanyREMOVE");
                        AtmCompanyGen(player);
                        NAPI.Data.SetEntityData(player, "ATMTYPE", 5);
                        break;
                    case 43: // зачисление
                        if (string.IsNullOrEmpty(acc.Org)) return;
                        Bank.Data bankf = Bank.Accounts[Organization.OCore.OrgListNAME[acc.Org].BankID];
                        if (acc.Money < +Math.Abs(amount))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Не достаточно средств!", 3000);
                            return;
                        }
                        MoneySystem.Wallet.Change(player, -Math.Abs(amount));
                        MoneySystem.Bank.Change(Organization.OCore.OrgListNAME[acc.Org].BankID, +Math.Abs(amount));
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы перевели на счёта компании: {amount}$!", 3000);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"company({acc.Org})", Math.Abs(amount), $"atmCompanyADD");
                        AtmCompanyGen(player);
                        NAPI.Data.SetEntityData(player, "ATMTYPE", 5);
                        break;
                    case 44:
                        ...
                        if(Bank.Accounts[bank].Type != 1 && Bank.Accounts[bank].Type != 3 && Main.Players[player].AdminLVL == 0) {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Счет не найден!", 3000);
                            Trigger.PlayerEvent(player, "closeatm");
                            return;
                        }
                        ...
                        if (Bank.Accounts[bank].Type == 3)
                        {
                            foreach (Organization.OCore.Organization org in Organization.OCore.OrgList.Values)
                            {
                                if (org.BankID == bank)
                                {
                                    Player ply = NAPI.Player.GetPlayerFromName(org.Owner);
                                    if (ply != null)
                                    {
                                        Notify.Send(ply, NotifyType.Alert, NotifyPosition.BottomRight, $"На счёт компании перевели {amount}$ <{player.Name}>", 5000);
                                    }
                                }
                            }
                        }
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        Organization.OCore.Organization org = Organization.OCore.OrgListNAME[acc.Org];
                        switch (index)
                        {
                            case -1:
                                Trigger.PlayerEvent(player, "atmOpen", "[1,0,0]");
                                ...
                                return;
                            case 0:
                                Trigger.PlayerEvent(player, "atmOpen", $"[2,'{Bank.Accounts[org.BankID].Balance}/{Convert.ToInt32(org.GetNalog()) * 24 * 7}$','Сумма зачисления']");
                                Trigger.PlayerEvent(player, "setatm", $"{org.BankID}", org.Name, Bank.Accounts[org.BankID].Balance, "");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", 43);
                                return;
                            case 1:
                                Trigger.PlayerEvent(player, "atmOpen", $"[2,'{Bank.Accounts[org.BankID].Balance}/{Convert.ToInt32(org.GetNalog()) * 24 * 7}$','Сумма вывода']");
                                Trigger.PlayerEvent(player, "setatm", $"{org.BankID}", org.Name, Bank.Accounts[org.BankID].Balance, "") ;
                                NAPI.Data.SetEntityData(player, "ATMTYPE", 42);
                                return;
