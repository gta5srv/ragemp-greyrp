    ...
    public class Manager : Script
    {
        ...
        public static void OpenFamilyMenu(Player player)
        {
            try
            {
                ...
                foreach(var pair in UnitedRP.Core.VehicleManager.getAllPlayerVehicles(Main.PlayerNames[ family.Leader]))
                    pairs.Add( new string[] { UnitedRP.Core.ParkManager.GetNormalName(UnitedRP.Core.VehicleManager.Vehicles[pair].Model), pair, Convert.ToString( family.Vehicles != null && family.Vehicles.ContainsKey(pair) ? family.Vehicles[pair] : 0 ) });
                ...
                UnitedRP.Trigger.PlayerEvent(player, "openFamilyMenu", json);
            }
            catch (Exception e) 
            { 
        ...
        static void fcSetMoney(Player player)
        {
            try
            {
                ...
                UnitedRP.Trigger.PlayerEvent(player, "openInput", $"Положить деньги в казну семьи ({Family.GetFamilyToCid(player).Money}$)", "Введите сколько вы хотите положить", 30, "setMfamily");
            }
            catch { }
        }
        ...
        static void fcGetMoney(Player player)
        {
            try
            {
                ...
                UnitedRP.Trigger.PlayerEvent(player, "openInput", $"Снять деньги с казны семьи ({Family.GetFamilyToCid(player).Money}$)", "Введите сколько вы хотите снять", 30, "getMfamily");
            }
            catch { }
        }
        ...
        static void fcBuyitem(Player player, int index)
        {
            try
            {
                ...
                Family family = UnitedRP.Families.Family.GetFamilyToCid(player);
        ...
        public static void LoadFamilyMembersToMenu(Player player)
        {
            try
            {
                ...
                UnitedRP.Trigger.PlayerEvent(player, "loadfamilymemberstomenu", json);
            }
            catch (Exception e)
            {
                Log.Write("Loaded Error \n" + e.ToString(), nLog.Type.Error);
        ...
        public static void OpenCreatorFamilyMenu(Player player)
        {
            try
            {
                ...
                string json = Newtonsoft.Json.JsonConvert.SerializeObject(data);
                UnitedRP.Trigger.PlayerEvent(player, "openCreatorFamilyMenu", json);
            }
            catch (Exception e)
            {
                Log.Write(e.ToString(), nLog.Type.Error);
        ...
        public static void LoadListFamilies(Player player)
        {
            try
            {
                ...
                UnitedRP.Trigger.PlayerEvent(player, "loadlistfamilies", "server", json);
            }
            catch (Exception e)
            {
                Log.Write(e.ToString(), nLog.Type.Error);
