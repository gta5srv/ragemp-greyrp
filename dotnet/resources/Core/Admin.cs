...
namespace UnitedRP.Core
{
    class Admin : Script
    {
        ...
        public static void sendRedbucks(Player player, Player target, int amount)
        {
            ...
            Trigger.PlayerEvent(target, "starset", Main.Accounts[target].RedBucks);
			
			MySQL.Query($"update `accounts` set `redbucks`={Main.Accounts[player].RedBucks} where `login`='{Main.Accounts[player].Login}'");

            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы отправили {target.Name} {amount} UP", 3000);
            Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"+{amount} UP", 3000);
        ...
        #region Demorgan
        public static Vector3 DemorganPosition = new Vector3(1675.039, 2587.174, 44.46871);
