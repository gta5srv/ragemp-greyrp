...
namespace UnitedRP.Core
{
    class Rentcar : Script
    {
        ...
        public static void RespawnCar(Vehicle vehicle)
        {
            ...
            NAPI.Data.SetEntitySharedData(vehicle, "PETROL", 150);
        ...
        public void Event_OnPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if (vehicle.GetData<Player>("DRIVER") == null)
                {
                    ...
                    Trigger.PlayerEvent(player, "openDialog", "RENT_CAR", $"Вы хотите арендовать этот транспорт за ${price}?");
                }
                else
                {
        ...
        public void Event_OnPlayerExitVehicle(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"Через 15 минуты аренда транспорта закончится", 3000);
        ...
        private void timer_playerExitRentVehicle(Player player, Vehicle vehicle)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    if (NAPI.Data.GetEntityData(player, "RENT_EXIT_TIMER_COUNT") > 900)
                    {
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Срок аренды автомобиля закончился", 3000);
                        Timers.Stop(NAPI.Data.GetEntityData(player, "RENT_CAR_EXIT_TIMER"));
                        NAPI.Data.ResetEntityData(player, "RENT_CAR_EXIT_TIMER");
                        if (player.HasData("TRACTOR")) { player.GetData<Vehicle>("TRACTOR").Delete(); player.ResetData("TRACTOR"); return; }
                        RespawnCar(vehicle);
        ...
        public static void RentCar(Player player)
        {
			...
			Utils.QuestsManager.AddQuestProcess(player, 12);
