            ...
            public void Send(Player someone = null)
            {
                if (someone == null)
                {
                    foreach (Player target in NAPI.Pools.GetAllPlayers())
                    {
                        ...
                        Trigger.PlayerEvent(target, "addreport", ID, Author, Question);
                    }
                }
                else
                {
                    ...
                    Trigger.PlayerEvent(someone, "addreport", ID, Author, Question);
                }
            }
        }
...
SELECT * FROM questions;";
        ...
        public void ReportTake(Player Player, int id, bool retrn = false)
        {
            ...
            foreach (Player target in NAPI.Pools.GetAllPlayers())
            {
                ...
                else Trigger.PlayerEvent(target, "setreport", id, Player.Name);
            }
        }
        ...
        public static void AddReport(Player player, string question)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@pn", player.Name + $"[{player.Value}]");
                ...
                {
                    ID = id,
                    Author = player.Name + $" [{player.Value}]",
                    Question = question,
                    BlockedBy = "",
                    Response = "",
                    Status = false,
        ...
        private static void AddAnswer(Player player, int repID, string response)
        {
            try
            {
                ...
                var split = Reports[repID].Author.Split(" ");

                try
                {
                    Player target = NAPI.Player.GetPlayerFromName(split[0]);
                    if (target is null)
                    {
                ...
                cmd.Parameters.AddWithValue("@resp", player.Name + $"[{player.Value}]");
                ...
                Reports[repID].Author = player.Name + $"[{player.Value}]";
