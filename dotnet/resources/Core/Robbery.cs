...
using System.Linq;
using System.Timers;
using UnitedRP.MoneySystem;
using UnitedRP.Fractions;
using UnitedRP.GUI;
...
namespace UnitedRP.Core
{
    public class SafeMain : Script
    {
        ...
        public static int SafeRespawnTime = 1800;
        ...
        public static bool AllowSafe = false;
        public static bool Mafia = false;
        ...
        public static int[] AllowMafia = new int[] { 10, 11, 12, 13 };
        ...
        public static void startSafeDoorCracking(Player player)
        {
            ...
            else if (!AllowMafia.Contains(Main.Players[player].FractionID))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны состоять в мафии", 3000);
                return;
            }
            ...
            if (!AllowSafe)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Невозможно взломать хранилище", 3000);
                return;
            }
            if (isCracking)
            {
        ...
        public void onPlayerEnterVehicle(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                if ((player.HasData("HAND_MONEY") || player.HasData("HEIST_DRILL")) && player.VehicleSeat == 0)
                {
        ...
        public static void openSafe(Player player, params object[] arguments)
        {
            try
            {
                ...
                {
                    Trigger.PlayerEvent(player, "dial", "close");
                    return;
                }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        Trigger.PlayerEvent(player, "dial", "open", safe.LockAngles[stage], true);
        ...
        {
            if (player.HasData("temp_SafeID") == false) return;
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Возможно открыть только с 7:00 до 00:00", 3000);
                    return;
                }
                ...
                Trigger.PlayerEvent(player, "dial", "open", safe.LockAngles[0]);
				...
				//Manager.sendWaypoint(9, player.Position);

                if (NowRobberyID != safe.ID) NextRobbery = DateTime.Now.AddMinutes(10);
                ...
                if (DateTime.Now >= safe.BlipSet)
                {
                    ...
                    foreach (var p in NAPI.Pools.GetAllPlayers())
                    {
                        ...
                        Trigger.PlayerEvent(p, "changeBlipAlpha", safe.Blip, 255);
                        Trigger.PlayerEvent(p, "createWaypoint", safe.Position.X, safe.Position.Y);
                    }
                    safe.BlipSet = DateTime.Now.AddMinutes(10);
                    NAPI.Task.Run(() => {
                        try
                        {
        ...
        public static void MoneyFlow(Player player)
        {
            ...
            {
                NAPI.Chat.SendChatMessageToPlayer(player, "~b~[Барыга] ~w~Чё ты тут забыл чепух?");
                return;
            }
            var all_money = 0;
            ...
            player.ResetData("HAND_MONEY");
            Wallet.Change(player, (int)(all_money * 1));
            GameLog.Money($"server", $"player({Main.Players[player].UUID})", (int)(all_money * 1), $"moneyFlow");
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы отмыли {(int)(all_money * 1)}$. Барыга взял {(int)(all_money * 0.2)}$ за свои услуги", 3000);
        }
        ...
        {
            Trigger.PlayerEvent(player, "mavrshop");
        }
        [RemoteEvent("mavrbuy")]
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 1:
                        ...
                        if (!Wallet.Change(player, -1420))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно денег", 3000);
                            return;
                        }
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 1420, $"buyMavr(drill)");
                    ...
                    case 2:
                        ...
                        Wallet.Change(player, -15);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 15, $"buyMavr(lockpick)");
                        ...
                        return;
                    case 3:
                        if (Main.Players[player].Money < 85)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно денег", 3000);
                            return;
                        }
                        ...
                        Wallet.Change(player, -85);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 85, $"buyMavr(armylockpick)");
                        ...
                        return;
                    case 4:
                        if (Main.Players[player].Money < 43)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно денег", 3000);
                            return;
                        }
                        ...
                        Wallet.Change(player, -43);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 43, $"buyMavr(cuffs)");
                        ...
                        return;
                    case 5:
                        if (Main.Players[player].Money < 72)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно денег", 3000);
                            return;
                        }
                        ...
                        Wallet.Change(player, -72);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 72, $"buyMavr(pocket)");
                    ...
                    case 6:
                        ...
                        if (Main.Players[player].Money < 14300)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Недостаточно средств", 3000);
                            return;
                        }
                        Wallet.Change(player, -14300);
                        GameLog.Money($"player({Main.Players[player].UUID})", $"server", 14300, $"buyMavr(wanted)");
        ...
        public void Destroy(bool check_players = false)
        {
            if (check_players)
            {
                foreach (var player in NAPI.Pools.GetAllPlayers())
                {
                    ...
                    Trigger.PlayerEvent(player, "SetSafeNearby", false);
                    player.ResetData("temp_SafeID");
                }
            }
