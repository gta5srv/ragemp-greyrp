...
namespace UnitedRP.Houses
{
    ...
    class House
    {
        ...
        public House(int id, string owner, int type, Vector3 position, int price, bool locked, int garageID, int bank, List<string> roommates, int apart)
        {
            ...
            else if (UnitedRP.Families.Family.FalimyHouses.ContainsKey(ID))
            {
                blip = NAPI.Blip.CreateBlip(84, Position, 0.7f, 12, UnitedRP.Families.Family.FalimyHouses[ID], 255, 0, true, 0, 0);
            }
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 2:
                        if (house.Locked)
                        {
                            if (UnitedRP.Families.Family.FalimyHouses.ContainsKey(house.ID))
                                if (UnitedRP.Families.Family.FamilyNames[UnitedRP.Families.Family.FalimyHouses[house.ID]] == Main.Players[player].FamilyCID)
                                {
                                    house.SendPlayer(player);
                                    return;
                                }
