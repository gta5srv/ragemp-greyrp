    ...
    class Hotel : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (var pos in HotelEnters)
                {
                    ...
                    var colshape = NAPI.ColShape.CreateCylinderColShape(pos, 0.5f, 1f, 0);
                ...
                foreach (var pos in CarsGet)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pos - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f, new Color(255, 255, 255, 220));
                    NAPI.TextLabel.CreateTextLabel("~b~Аренда скутера", pos + new Vector3(0, 0, 0.5), 5f, 0.4f, 0, new Color(255, 255, 255));
                    HotelID++;
                }
            }
                ...
                case 50:
                    if (player.GetData<int>("HOTEL_ID") == Main.Players[player].HotelID)
                    {
                        if (player.HasData("HOTELCAR"))
                            NAPI.Entity.DeleteEntity(player.GetData<Vehicle>("HOTELCAR"));
        ...
        private static void callback_hotelmanage(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            switch (item.ID)
            {
                case "extend":
                    MenuManager.Close(player);
                    Trigger.PlayerEvent(player, "openInput", $"Продлить аренду ({HotelRent}$/ч)", "Введите количество часов", 1, "extend_hotel_rent");
                    return;
                case "moveout":
