    ...
    methods: {
        left: function(type){
            if(type==0){ //style
                ...
                mp.trigger('premclothes','style',this.indexS)
            } else { //color
                this.indexC--
                if(this.indexC < 0) this.indexC = this.colors.length - 1
                //console.log(this.indexC)
                mp.trigger('premclothes','color',this.indexC)
            }
        },
        right: function(type){
            if(type==0){ //style
        ...
        buy: function(){
            //console.log('buy')
            mp.trigger('buypremclothes')
        },
        exit: function(){
            //console.log('exit')
            this.reset()
            mp.trigger('closepremclothes')
        },
        reset: function(){
            this.price=-1
            this.indexS=0
