...
{
	setTimeout(() => {
		auth.execute(`slots.remember=true`);
		auth.execute(`slots.mylogin="${mp.storage.data.login}"`);
		auth.execute(`slots.mypass="${mp.storage.data.pass}"`);
	}, 400)
}
...
// events from cef
mp.events.add('signin', function (authData, remember) {
...
mp.events.add('ready', function () {
    ...
    global.menuClose();
    mp.game.cam.renderScriptCams(false, true, 3000, true, true); //вместо нуля указать секунды для полета с нема при спавне
